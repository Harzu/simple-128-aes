...
function generate16ByteBlocks(input) {
  const blocks = []
  while (input.length > 0) {
    const block = new Array(16)
    for (let i = 0; i < 16; i++) {
      block[i] = i < input.length ? input.charCodeAt(i) : 0
    }
    blocks.push(block)
...
function parseBytesToString(bytes) {
  let result = []
  for (let byteBlock of bytes) {
    result.push(
      byteBlock
